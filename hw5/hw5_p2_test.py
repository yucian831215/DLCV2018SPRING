...
def Action_Recognition(inception_model, lstm_model, video_frame, batch_size):
	...
	# Reshape video_feature
	video_feature = video_feature.reshape((1, ) + video_feature.shape)

	# Use fc_model to get the action label
	label_softmax = lstm_model.predict(video_feature)
...
if __name__ == '__main__':
	...
	for index in range(len(valid_video_list['Video_index'])):
		...
		video_frame = normalize_frame(video_frame)
		labels.append(Action_Recognition(extract_features_model, lstm_model, video_frame, 10))
	print('\n')

	labels = np.array(labels)
	labels = labels.reshape(labels.shape + (1,))
	np.savetxt(os.path.join(output_path, 'p2_valid.txt'), labels, fmt='%d')
